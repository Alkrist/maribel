...
public class UIVAO {
	...
	public void addData(Mesh mesh){
			...
			glBufferData(GL_ARRAY_BUFFER, Util.createFlippedBufferSOA(mesh.getVertices()), GL_DYNAMIC_DRAW);
	...
	public void update(Vector2f[] texCoords){
		glBindVertexArray(vaoId);
		glBindBuffer(GL_ARRAY_BUFFER, vbo);
		glBufferSubData(GL_ARRAY_BUFFER, vertices * offset, Util.createFlippedBuffer(texCoords));
		glBindVertexArray(0);
	}
	
	public void update(List<Vector2f> texCoords){
		glBindVertexArray(vaoId);
		glBindBuffer(GL_ARRAY_BUFFER, vbo);
		glBufferSubData(GL_ARRAY_BUFFER, vertices * offset, Util.createFlippedBuffer(texCoords));
		glBindVertexArray(0);
