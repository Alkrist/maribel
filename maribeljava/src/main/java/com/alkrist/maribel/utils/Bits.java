...
/** NOTE: this original bitset is found on Libgdx/Bits
...
public class Bits {
	...
	/**Create a bitfield with a specified length
	 * 
	 * @param number - length
	 */
	public Bits(int number) {
		checkCapacityExtended(number >>> 6);
	}
	
	private void checkCapacityExtended(int length) {
	...
	/**Get the state of the element on index: is it set or not?
	 * 
	...
	public void or (Bits other) {
		...
		if (commonWords < other.bits.length) {
			checkCapacityExtended(other.bits.length);
