...
public class ShadowBox {
	...
	protected ShadowBox(Matrix4f lightViewMatrix, Camera camera, DisplayManager manager) {
	...
	protected void update() {
		...
		Vector4f transformed = Matrix4f.transform(rotation, FORWARD, null);
		Vector3f forwardVector = new Vector3f(transformed.x, transformed.y, transformed.z);
		...
		toNear.scale(Settings.CURRENT.NEAR_PLANE);
	...
	protected Vector3f getCenter() {
		...
		Vector4f transformed = Matrix4f.transform(invertedLight, cen, null);
	...
	private Vector4f[] calculateFrustumVertices(Matrix4f rotation, Vector3f forwardVector,
		...
		Vector4f transformed = Matrix4f.transform(rotation, UP, null);
		Vector3f upVector = new Vector3f(transformed.x, transformed.y, transformed.z);
	...
	private Matrix4f calculateCameraRotationMatrix() {
		...
		rotation.rotate((float) Math.toRadians(-camera.yaw), new Vector3f(0, 1, 0));
		rotation.rotate((float) Math.toRadians(-camera.pitch), new Vector3f(1, 0, 0));
		return rotation;
	}
	
	private void calculateWidthsAndHeights() {
		farWidth = (float) (SHADOW_DISTANCE * Math.tan(Math.toRadians(Settings.CURRENT.FOV)));
		nearWidth = (float) (Settings.CURRENT.NEAR_PLANE
				* Math.tan(Math.toRadians(Settings.CURRENT.FOV)));
		farHeight = farWidth / getAspectRatio();
		nearHeight = nearWidth / getAspectRatio();
	}
	
	private float getAspectRatio() {
		return (float) manager.getWidth() / (float) manager.getHeight();
	}
}
