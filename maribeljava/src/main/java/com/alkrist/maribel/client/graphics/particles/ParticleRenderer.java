...
public class ParticleRenderer {
	...
	public void render(Camera camera) {
		...
		for (Texture texture : particles.keySet()) {
			...
			sortHighToLow(particleList, camera);
	...
	private static void sortHighToLow(List<Particle> list, Camera camera) {
		for (int i = 1; i < list.size(); i++) {
			Particle item = list.get(i);
			if (item.getDistance(camera) > list.get(i - 1).getDistance(camera))
				sortUpHighToLow(list, i, camera);
		}
	}

	private static void sortUpHighToLow(List<Particle> list, int i, Camera camera) {
		Particle item = list.get(i);
		int attemptPos = i - 1;
		while (attemptPos != 0 && list.get(attemptPos - 1).getDistance(camera) < item.getDistance(camera)) {
			attemptPos--;
		}
		list.remove(i);
